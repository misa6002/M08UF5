...
$sql = "SELECT i.nombre, i.ruta, i.fecha FROM imagenes i JOIN usuarios u ON u.id = i.usuario_id WHERE u.nombre = '$nombre' AND fecha >= NOW() - INTERVAL 1 WEEK ORDER BY fecha DESC LIMIT $pagina OFFSET $offset";
...
?>
    ...
    <?php
        $sql_total = "SELECT COUNT(*) as total FROM imagenes i JOIN usuarios u ON u.id = i.usuario_id WHERE u.nombre = '$nombre' AND fecha >= NOW() - INTERVAL 1 WEEK ORDER BY fecha DESC LIMIT $pagina";
