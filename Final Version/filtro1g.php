...
$sql = "SELECT g.nombre, g.grabaciones, g.fecha FROM grabaciones g JOIN usuarios u ON u.id = g.usuario_id WHERE u.nombre = '$nombre' AND fecha >= NOW() - INTERVAL 1 HOUR ORDER BY grabaciones DESC LIMIT $pagina OFFSET $offset";
...
<?php
    $sql_total = "SELECT COUNT(*) as total FROM grabaciones g JOIN usuarios u ON u.id = g.usuario_id WHERE u.nombre = '$nombre' AND fecha >= NOW() - INTERVAL 1 HOUR ORDER BY grabaciones DESC LIMIT $pagina";
